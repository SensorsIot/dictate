...
| O-03 | Is `scribe_v2` the correct model identifier, and is `scribe_v1` still accepted? | ElevenLabs docs / a live call | FR-10.2 default | `scribe_v2`, from the API reference. Verifiable in one request once a key exists |
...
Nothing yet. When an item is answered, move it here with the answer and the date,
and update the FSD tag from `[assumed]` to `[user]` in the same commit.
